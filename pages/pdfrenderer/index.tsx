...
interface SelectedProduct {
  ...
  pozita: string;
  njesiaMatese: string;
  sasia: number; // Changed the type to number
  cmimiNjesisePaTVSH: number;
  [key: string]: string | number;
}

interface QuantityInputTableProps {
  predefinedProducts: SelectedProduct[];
  productQuantities: number[];
  handleQuantityChange: (index: number, quantity: number) => void;
}

const QuantityInputTable: React.FC<QuantityInputTableProps> = ({
  predefinedProducts,
  productQuantities,
  ...
  const generatePDF = () => {
    if (invoiceNumber || subject || issueDate || dueDate !== null) {
      ...
      predefinedProducts.forEach((product, index) => {
        ...
        const totalForTS = quantity * product.cmimiNjesisePaTVSH;
        totalSum += totalForTS;
      });

      const vatPercentage = 0.18;
      const vatAmount = totalSum * vatPercentage;
