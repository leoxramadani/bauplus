    ...
    inflow: z
      .object({
        ...
        dosja: z.string().max(20, {
          message: "The length of 'dossier' must be 20 characters or shorter",
        }),
      ...
      .superRefine(
        (
          ...
          const { vleraTVSH, shumaTVSH, dataNeDokument, afatiPageses } = arg;
  
          if (vleraTVSH > shumaTVSH) {
            ctx.addIssue({
              code: "custom",
              message: "'Value of TAX' must be smaller than 'Sum with TAX'",
              path: ["vleraTVSH"],
            });
            if (dataNeDokument >= afatiPageses) {
              ctx.addIssue({
                code: "custom",
                message: "Date in document must be smaller than Payment deadline",
                path: ["dataNeDokument"],
              });
              return false;
            }
